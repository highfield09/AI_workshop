...
def interface_intro(name: str, guidance: str, extra: str = ""):
    ...
    return markdown(
        ...
        + panel(
            ...
            "<b>_for_TRAINER/llm_workshop/worksheet.py</b> explicitly choose the path; "
...
- [ ] The result is saved at `_for_STUDENT/outputs/notebook1/catalogue.html`.
...
{panel(
    ...
    "<b>_for_STUDENT/outputs/notebook1/catalogue.html</b>.",
    "success",
)}
"""
        ),
    ...
    for index, (stem, title, questions) in enumerate(WORKBOOKS):
        ...
        for cell in deepcopy(cells[boundaries[index]:boundaries[index + 1]]):
            ...
            cell.source = cell.source.replace(
                ...
                "tasks → workbook_answers.json", f"_for_STUDENT → tasks → {stem} → answers.json"
            ).replace("_for_STUDENT/outputs/notebook1/", "_for_STUDENT/outputs/05_shopping_catalogue/")
            if stem == "05_shopping_catalogue":
...
def main() -> None:
    for stem, notebook in build_workbooks().items():
        path = ROOT / "_for_STUDENT" / "notebooks" / f"{stem}.ipynb"
