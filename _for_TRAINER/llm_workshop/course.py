...
WORKBOOKS = (
    ...
    ("06_receipt_ocr", "Read one invoice into Excel", (19, 20)),
)
...
def answer_path(workbook: str) -> Path:
    ...
    tasks = Path(demo_root) if demo_root else ROOT / "_for_STUDENT" / "tasks"
...
def answer_label(path: Path) -> str:
    ...
    if path.name == "answers.json" and path.parent.name in {x[0] for x in WORKBOOKS}:
        return f"_for_STUDENT/tasks/{path.parent.name}/answers.json"
    try:
